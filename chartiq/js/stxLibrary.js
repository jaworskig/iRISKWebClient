	...
	function _stxLibrary_js(_exports) {
		...
		STX.Studies.displayElderRay=function(stx, sd, quotes){
			...
			function drawBar(i,reduction,output,hist){
				...
				stx.chart.context.fillRect(Math.floor(stx.pixelFromBar(i, panel.chart)+negativeOffset)+myWidth*reduction,
						Math.floor(y), 
		 ...
		 * 
		 * Note: by default the renderer will display lines as underlays. As such, they will appear bellow the chart ticks and any other studies or drawings.
		 * 
		...
		*/
		...
		STX.Drawing.fibonacci.prototype.render=function(context){
			...
			this.stx.canvasFont("stx_yaxis", context); // match font form y axes to it looks cohesive
		 ...
		 * the QuoteFeed to a proxy server that injects the token.
		 * 
		 ...
		 * If Xignite can not support the access-control-allow-origin header, you can always proxy the requests trough your own servers and add your own header
		 *
		 ...
		 *
		 * 		Also add the following override before you attach to the feed:
		 * 
		 ...
		 * STX.QuoteFeed.Xignite.Utility.overrides.server="myproxyserver.com/proxystring";
		 * STX.QuoteFeed.Xignite.Utility.overrides.path="xignite";  
		 * stxx.attachQuoteFeed(new STX.QuoteFeed.Xignite(myToken));
		 * 
		...
		STX.QuoteFeed.Xignite.Utility={
			...
			url: function(api,args,params,token){
				...
				if(api.host instanceof Object){
					u+=(override.protocol)?override.protocol:api.host.protocol;
					u+=(override.server)?override.server:api.host.server;
					u+=(override.path)?override.path:api.host.path;
				}else{
					u+=api.host;
				}
