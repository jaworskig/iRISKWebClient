...
(function(){
	...
	function _stxAdvanced_js(_exports) {
		...
		/**
		 * Comparisons can either be "compare", "overlay" or "absolute". The UI defaults to "compare" which produces a y-axis with relative percentage
		 * changes. "overlay" overlays the series so that the axis is not shared.
		 * 
		 ...
		 * @since 07/01/2015 added parameters argument.
		 */
		STX.Comparison.add=function(stx, compareSymbol, cb, displaySymbol, parameters){
			...
			if(compareSymbol==stx.chart.symbol && !parameters.force) return;
		 ...
		 * Once you have gathered your data call STX.addMemberToMasterdata to add it to the masterData array:
       	 * STX.addMemberToMasterdata(stx, comparisonSymbol, comparisonData, null, null, parameters.field);
	 	 * Do not set parameters.data to {useDefaultQuoteFeed:true} when overriding and not using a quotefeed!
	 	 * Then call addSeries followed by parameters.fetchCallBack(err) to complete the loading once you have all of the data.
		 * The data you fetch should be in the standard chart JSON format.
		 * Once your data is fetched, call cb();
		 * @memberOf STX.Comparison
		 * @version ChartIQ Advanced Package
		 */
		STX.Comparison.fetch=function(stx, comparisonSymbol, parameters, cb){
			// fetch comparison data here
			//STX.addMemberToMasterdata(stx, comparisonSymbol, comparisonData, null, null, parameters.field);
			if(!parameters.data) parameters.data={useDefaultQuoteFeed:true};
			stx.addSeries(comparisonSymbol, parameters, cb);
			//parameters.fetchCallBack();
			//if(cb) cb();
		};
...
*/		
		 ...
		 * 		// put ytour code to establish the behavior of your UI.
		...
		STX.Studies.displayIchimoku=function(stx, sd, quotes){
		    ...
		    for(i=0;i<sd.futureA.length-1;i++){ //creates array of future intersection points so clouds project into the future
		...
		STX.Studies.displayMFI=function(stx, sd, quotes){
			...
			for(var i=0;i<quotes.length;i++){
				...
				stx.chart.context.fillRect(Math.floor(stx.pixelFromBar(i, panel.chart)+negativeOffset),
											Math.floor(y), 
											Math.floor(myWidth),
		...
		/**
		 * Channel drawing tool. Creates a channel with 2 parallel line segments.
		 * 
		 * It inherits its properties from {@link STX.Drawing.segment}.
		 * @constructor
		...
		STX.Drawing.channel.prototype.click=function(context, tick, value){
			...
			if(this.p2!==null){
				this.setPoint(2, tick, value, panel.chart);
				return true;
			}
			this.setPoint(1, tick, value, panel.chart);
		...
		STX.Drawing.channel.prototype.render=function(context){
			...
			var y2=null;
			if(this.p2) y2=this.stx.pixelFromValueAdjusted(panel, this.p2[0], this.p2[1]);
			...
			if(y2 && fillColor && !STX.isTransparent(fillColor) && fillColor!="auto"){
				context.beginPath();
				context.moveTo(x0,y0);
				context.lineTo(x1,y1);
				context.lineTo(x1,y2);
				context.lineTo(x0,y0+(y2-y1));
				context.closePath();
			...
			if(y2) this.stx.plotLine(x0, x1, y0+(y2-y1), y2, color, "segment", context, panel, parameters);
			...
			if(this.highlighted){
				...
				this.littleCircle(context, x1, y2, p2Fill);  //not an error, should be d1 here
			}		
		};
		...
		STX.Drawing.gartley.prototype.reconstructPoints=function(){
			...
			for(var a=0;a<this.pts.length;a+=3){
				...
				this.points.push([this.stx.tickFromDate(STX.yyyymmddhhmm(d),panel.chart),this.pts[a+2]]);
			}
		};
		...
		/**
		 * Placement functions are responsible for positioning markers in their holder. firstTick and lastTick
		 * can be used as a hint as to whether to display a marker or not.
		 * @param  {Object} params Parameters for the array
		 * @param {STXChart} stx The chart object
		 * @param {Array} arr The array of markers to place
		 * @param {Object} panel The panel where the markers are to be placed
		 * @param {Number} firstTick The first tick displayed on the chart
		 * @param {Number} lastTick The last tick displayed on the chart
		...
		STX.Marker.AboveCandle.placementFunction=function(params){
			...
			for(var i=0;i<params.arr.length;i++){
				...
				if(marker.params.xPositioner=="bar"){
					...
					node.style.left=Math.round(stx.pixelFromBar(marker.params.x, chart)-marker.clientWidth/2)+"px";
				}else{
				...
				if(useHighs){
					bottom=Math.round(bottom-stx.pixelFromPriceTransform(quote.High, panel, yAxis))+"px";
				}else{
					bottom=Math.round(bottom-stx.pixelFromPriceTransform(quote.Close, panel, yAxis))+"px";
				}
				if(i==params.arr.length-3) console.log(marker.tick,marker.leftpx);
