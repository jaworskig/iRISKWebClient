	...
	function _stx_js(_exports, _stxThirdParty) {
		...
		*/
		...
		STX.safeClickTouch=function(div, fc, params){
			...
			if("onpointerup" in document) div.addEventListener("pointerup", fc3);
			if(!div.safeClickTouchEvents) div.safeClickTouchEvents=[];
			div.safeClickTouchEvents.push({"mouseup":fc1,"touchend":fc2,"pointerup":fc3});
		};
		...
		STX.clearSafeClickTouches=function(div){
			...
			for(var i=0;i<div.safeClickTouchEvents.length;i++){
				...
				if("onmouseup" in document) div.removeEventListener("mouseup", fc.mouseup);
				if("ontouchend" in document) div.removeEventListener("touchend", fc.touchend);
				if("onpointerup" in document) div.removeEventListener("pointerup", fc.pointerup);
			}
...
Heikin-Ashi,Heikin-Ashi,Heikin-Ashi,Heikin-Ashi,Heikin-Ashi,Heikin-Ashi,Heikin-Ashi,Heikin-Ashi,Heikin-Ashi,平均酒吧,平均足
...
*/});
		...
		STX.Market.prototype.isMarketOpen=function(dt){
			...
			if(!sessions.length) return false;	// Market closed today
		
			var replaceSessions=this.getHoliday(newdt);
			if(replaceSessions) sessions=replaceSessions;
			if(!sessions.length) return false;	// Holiday today
		...
		STX.Market.Iterator.prototype.increment=function(){
			...
			if(this.interval=="minute"){
				...
				for(i=0;i<1000;i++){	// Use for loop to prevent infinite loop in case of bugs...
					day=future.getDay();
					var sessions=this.market.sessions[day];
					if(!sessions.length){	// market closed today
						newSession=true;
					...
					if(replaceSessions) sessions=replaceSessions;
					if(!sessions.length){ // Holiday today
						newSession=true;
						future.setDate(future.getDate()+1);
						this.sessionPointer=0;
						continue;
				...
				for(i=0;i<sessions.length;i++){
					...
					if(minutesAfterMidnight<session.close){ // If before a session closes then we want that session
						...
						if(minutesAfterMidnight<session.open){	// If before that session has opened then fast-forward to beginning of sesson
							...
							minutes=minutesAfterMidnight+(this.period-(minutesAfterMidnight-session.open)%this.period);	// ensure we land on a candle boundary
				...
				for(i=sessions.length-1;i>-1;i--){
					...
					if(minutesAfterMidnight>=session.open){ // If greater than a session open then we want that session
						...
						if(minutesAfterMidnight<session.close){	// Our initial date falls inside of a session
							minutes=minutesAfterMidnight-(minutesAfterMidnight-session.open)%this.period;	// Ensure we land on a candle boundary
							this.dt.setHours(Math.floor(minutes/60));
							this.dt.setMinutes(minutes-this.dt.getHours()*60);
		...
		STX.Market.Iterator.prototype.decrement=function(){
			...
			if(this.interval=="minute"){
				...
				for(i=0;i<20;i++){	// Use loop to prevent infinite loop in case of bugs...
					day=past.getDay();
					var sessions=this.market.sessions[day];
					if(!sessions.length){	// market closed today
						newSession=true;
		...
		/**
		 * Returns true if the symbol is a foreign symbol. This is dependent on the market data feed and should be overridden accordingly.
		...
		STX.LegacyMarket.nextPeriod=function(dt, interval, periods, stx, symbol,dataZone){
			...
			if(!stx.calendarAxis){
				if(STX.LegacyMarket.isForexFuturesSymbol(symbol)){
					...
					if(!STX.LegacyMarket.isMarketDay(symbol,nyDate) || nyDate.getHours()>endHour || (nyDate.getHours()==endHour && nyDate.getMinutes()>stx.chart.endMinute) || nyDate.getHours()===0){
		...
		STX.LegacyMarket.prevPeriod=function(dt, interval, periods, stx, symbol, dataZone){
			...
			if(!stx.calendarAxis){
				if(STX.LegacyMarket.isForexFuturesSymbol(symbol)){
					...
					if(!STX.LegacyMarket.isMarketDay(symbol,nyDate) || nyDate.getHours()<stx.chart.beginHour || (nyDate.getHours()==stx.chart.beginHour && nyDate.getMinutes()<stx.chart.beginMinute)){
		...
		STX.Studies.StudyDescriptor=function(name, type, panel, inputs, outputs, parameters){
			...
			if(!this.libraryEntry) this.libraryEntry={};
		...
		STX.Studies.prepareStudy=function(stx, study, sd, parameters){
			...
			if(study.underlay || sd.inputs.Underlay){
				stx.overlays[sd.name]=sd;
			}
			if(parameters && parameters.replaceID){
				// Remove any overlays that relied on the old panel ID name, for instance a moving average on RSI(14) is no   
		...
		STX.Studies.addStudy=function(stx, type, inputs, outputs, parameters, panelName){
			...
			var isPanelStudy=!(study.overlay || study.underlay);
		 ...
		 * @memberOf STX.Studies
		 * @example
		 * STX.Studies.quickAddStudy(stxx, "vol undr", {}, {"Up Volume":"#8cc176","Down Volume":"#b82c0c"});
		 */
		STX.Studies.quickAddStudy=function(stx, studyName, inputs, outputs, parameters){
			if(!parameters) parameters={};
			var sl=STX.Studies.studyLibrary[studyName];
			if(!inputs && sl && sl.inputs) inputs=STX.shallowClone(sl.inputs);
			if(!inputs) inputs={};
			if(!outputs && sl && sl.outputs) outputs=STX.shallowClone(sl.outputs);
		...
		STX.Studies.removeStudy=function(stx, sd){
			...
			for(var p in stx.plugins){
				if(p.indexOf("{"+sd.id+"}")>-1) delete stx.plugins[p];
			}
			if(sd.libraryEntry && (sd.libraryEntry.overlay || sd.libraryEntry.underlay)){
		...
		STX.Studies.displayIndividualSeriesAsLine=function(stx, sd, panel, name, quotes){
			...
			if(!sd.libraryEntry || sd.libraryEntry.overlay || sd.libraryEntry.underlay) labelDecimalPlaces=null; // will end up using the same as the chart itself
		    ...
		    for(var i=1;i<quotes.length;i++){
		        ...
		        if(!highs.length){
		            ...
		            if(!isNaN(hy1)){
		            	...
		            	x1=panel.left+(i==1?0:(i-1)*stx.layout.candleWidth + stx.offset + stx.micropixels);
		            	highs.push([x1,hy1]);
		            }
		        }
	            ...
	            if(highs.length || lows.length){
	            	...
	            	var x2=panel.left+i*stx.layout.candleWidth + stx.offset + stx.micropixels;
	            	if(highs.length){
		            	if(noSlopes){
		...
		STX.Studies.preparePeakValleyFill=function(stx, quotes, parameters){
		    ...
		    for(var i=1;i<quotes.length;i++){
		        ...
		        if(!points.length){
	            	var x1=(i-1)*stx.layout.candleWidth + stx.offset + stx.micropixels;
	            	if(i==1){
	            		var leftTick=stx.chart.dataSet.length-stx.chart.scroll;
						if(leftTick>0){
							var x0=panel.left+stx.offset-stx.layout.candleWidth;
		...
		STX.Studies.createHistogram=function(stx, sd, quotes, centered){
			...
			var negativeOffset=(panel.chart.tmpWidth-stx.layout.candleWidth)/2;
			for(var i=0;i<quotes.length;i++){
				var quote=quotes[i];
				if(!quote) continue;
				var x0=Math.floor(stx.pixelFromBar(i, panel.chart)+negativeOffset);
		...
		STX.Studies.volumeChart=function(stx, sd, colorMap, borderMap){
			...
			var negativeOffset=(panel.chart.tmpWidth-candleWidth)/2;
			...
			for(color in colorMap){
				...
				context.beginPath();
				var prevTop=b+0.5;
				var farLeft=(borderMap?panel.left+0.5:panel.left);
				...
				for(var i=0;i<colorMap[color].length;i++){
					...
					var x0,x1;
					if(borderMap){
						x0=Math.floor(prevRight+Math.floor(negativeOffset)+stx.micropixels)+0.5;
						x1=Math.floor(prevRight+candleWidth+Math.floor(negativeOffset)+stx.micropixels)+0.5;
						x0=Math.max(x0, farLeft);
					}else{
						x0=panel.left+Math.floor(i*candleWidth+stx.micropixels);
						x1=x0+panel.chart.tmpWidth;
					}

					context.moveTo(x0+0.5, b);
					context.lineTo(x1, b);
		...
		/**
		 * Base class for Renderers. A renderer is user to draw a complex visualization based on one or more "series" of data.
		 * @name  STX.Renderer
		 * @constructor
		 */
		...
		/**
		 * Add a series to the renderer. This assumes that the series data *is already in the dataSet* and simply connects it to the renderer with the specified parameters. See {@link STXChart.addSeries}.
		 * 
		...
		/**
		 * Removes all series from the renderer
		...
		STX.Drawing.prototype.dragToDraw=false; // Set to true when need to hold mouse down to draw; set to false for click on/off draw
		...
		/**
		 * Convenience function to destruct a chart container , eliminating all references and dependencies
		 * @since 07/01/2015
		 * @memberOf STX
		 */
		STX.destroy=function(){
		    stxx.styles={};
		    stxx.destroy();
		    stxx=null;
		    STX.ThemeManager.destroy();
		    STX.MenuManager.destroy();
		    STXChart.registeredContainers=[];
		    $$$(".stx-wrapper").parentNode.removeChild($$$(".stx-wrapper"));
		    $$$(".stxBodyOverlay").parentNode.removeChild($$$(".stxBodyOverlay"));   
		};
		...
		@param {boolean} config.cascade - set to true to allow the symbol lookup window to cascade on top of another window ( example: the comparison window in the advanced package).
		...
		@param {string} config.ciqLookupService.path Path to the external file containing the Lookup service. 
		@param {string} config.ciqLookupService.service The external file containing the Lookup service. 
		...
		*/
		...
		STX.DrawingToolbar.configurator={
				".stxToolbarFill":{			"measure":false, "line":false, "ray":false, "segment":false, "annotation":false, "horizontal":false, "vertical":false,									   "continuous":false, "fibonacci":false,									"freeform":false, "pitchfork":false},
				".stxToolbarLine":{},
				...
				"#stx-toolbar-settings":{	"measure":false, "line":false, "ray":false, "segment":false, "annotation":false, "horizontal":false, "vertical":false, "rectangle":false, "ellipse":false, "continuous":false,					  "channel":false, "gartley":false, "freeform":false, "pitchfork":false}
		};
		...
		STXChart.version=["Version 2015-07-01"];
		 ...
		 * <P><B>Valid values for vectorType: measure,annotation,fibonacci,horizontal,vertical,line,ray,segment,continuous,channel,gartley,rectangle,ellipse,freeform</B></P>
		 ...
		 * @example
		 * // here is an example on how to turn off the last price label before the inital axis has already been rendered
		 * var stxx=new STXChart({container:$$$(".chartContainer"), layout:{"candleWidth": 16, "crosshair":true}});
		...
		/**
		 * set to false to hide grid lines. See {@tutorial Gridlines and  labels} for additional details.
		 * @type {boolean}
		 * @default
		 * @memberOf  STXChart.YAxis
		 */
		...
		/**
		 * Set to true to right justify the yaxis (use with {@link STXChart.yaxisLeft})
		 * @type Boolean
		 * @default
		 * @memberOf  STXChart.YAxis
		 * @since  15-07-01
		...
		/**
		 * Set to true to put a rectangle behind the yaxis text (use with {@link STXChart.yaxisLeft})
		 * @type Boolean
		 * @default
		 * @memberOf  STXChart.YAxis
		 * @since  15-07-01
		...
		/**
		 * The width in pixels. This overrides {@link STXChart.yaxisWidth}
		 * 
		 * Visual Reference:
		 ...
		 * access for any study panel will follow the main chart axis as long as this is set to null.
		 * @type {string}
		 * @default
		 * @memberOf  STXChart.YAxis
		 * @since  15-07-01
		...
		STXChart.XAxis.prototype={
		     ...
		     * See {@tutorial Gridlines and  labels} for additional details.
		     * @type number 
		     * @default
		     * @memberof   STXChart.XAxis#
		     */			
		...
		STXChart.Chart.prototype={
			    ...
			    /**
			     * market opening hour in **EST** -- if not set  {@link STX.LegacyMarket.getHours} will be used
			     * @type number 
			     * @default
			     * @memberof   STXChart.Chart#
			     */
				beginHour:0,							// market opening hour in EST -- if not set  STX.LegacyMarket.getHours() will be used 
			    /**
			     * market opening minute in **EST** -- if not set  {@link STX.LegacyMarket.getHours} will be used
			     * @type number 
			     * @default
			     * @memberof   STXChart.Chart#
			     */
				beginMinute:0,							// market opening minute in EST -- if not set  STX.LegacyMarket.getHours() will be used 
			    /**
			     * market close hour in **EST** -- if not set  {@link STX.LegacyMarket.getHours} will be used
			     * @type number 
			     * @default
			     * @memberof   STXChart.Chart#
			     */
				endHour:23,								// market closing hour in EST -- if not set  STX.LegacyMarket.getHours() will be usedd 
			    /**
			     * market close minute in **EST** -- if not set  {@link STX.LegacyMarket.getHours} will be used
			     * @type number 
			     * @default
			     * @memberof   STXChart.Chart#
			     */
				endMinute:59,							// market closing minute in EST -- if not set  STX.LegacyMarket.getHours() will be used 
		...
		function STXChart(config){
			...
			this.layout={							
			     ...
			     * Available options are: "line", "candle", "bar", "wave", “colored_bar”, "colored_line", “hollow_candle”,”scatterplot”, "baseline_delta","mountain"
			     * @type string
			     * @default
			...
			*/			
		    ...
		    /**
		     * Set to true to have the Chart create missing data points for lightly traded stocks that may have missing ticks for the set interval. **Only executed on introday intervals (minutes)**
		     * @type boolean
		     * @default
		     * @alias cleanupGaps
		     * @memberOf  STXChart.prototype
		 ...
		 * @param {Boolean} [createObject] 	If true, then data elements fronm the data array are added as *objects* assigned to the label. Example: member[label]=data[element]; This behavior is mutually exclusive with `fields`.
		 ...
		 * @abstract
		 * @memberOf  STX.QuoteFeed
		 * @since 04-2015 -- must take into account the scenario where a date range is sent in the params (params.startDate && params.endDate) to fill in a gap in the masterData array. Usually used for series or studies.
		 */
		...
		STX.QuoteFeed.Demo.prototype.generateIntraday=function(params, cb){
			...
			for(var i=0;i<ticksToLoad ;i++){
				...
				newQuote.Date=STX.yyyymmddhhmm(now);
				...
				//}
				now = STX.LegacyMarket.prevPeriod(now, params.interval, params.period, params.stx, params.symbol, params.stx.dataZone);
				seed=newQuote.Close;
			}
			
			params.stx.calendarAxis = originalCalendarAxis;  // very important to restore the original state when done.

			cb({quotes:quotes.reverse(), moreAvailable:true}); // set moreAvailable to true so that the chart will request more when scrolling into the past. Set to false if at the end of data.
		};		
		...
		STX.QuoteFeed.Demo.prototype.generateIntradayRange=function(params, cb){
			...
			while (now <=params.endDate){
				...
				newQuote.Volume=Math.round(newQuote.Volume*params.period/500);
				quotes.push(newQuote);
				now = STX.LegacyMarket.nextPeriod(now, params.interval, params.period, params.stx, params.symbol, params.stx.dataZone);
				seed=newQuote.Close;
			}
		...
		STX.QuoteFeed.Demo.prototype.loadMore=function(params, cb){
			...
			var now = STX.LegacyMarket.prevPeriod(params.endDate, params.interval, params.period, params.stx, params.symbol, params.stx.dataZone);
